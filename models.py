...
class OkTest(models.Test):
    ...
    def run(self, env):
        ...
        for i, suite in enumerate(self.suites):
            ...
            test_obj = WithFlags(suite.cases[0].lines[1].strip('>>> '), env, suite.cases[0].lines[2].output[0],'') #jack
            print('calling')
            test_obj.eval_ans()
            print('The call: {}'.format(test_obj.eval_ans())) #jack
...
class Suite(core.Serializable):
    ...
    def _run_case(self, test_name, suite_number, case, case_number):
        ...
        print('{} > Suite {} > Case {}'.format(test_name, suite_number,
                                               case_number))
        print() #temporarily uncommented this line and the one above #jack

        #This command is where the test is being run from:
        #The #+ELLIPSIS is still in the code here.
        success = case.run()
        if success:
            print('-- OK! --')
        ...
        if not success or self.verbose:
            print(''.join(output_log)) #this could be the key???
            #print('output_log: {}'.format(output_log)) #jack
        if not success:
