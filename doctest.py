...
class DoctestSuite(models.Suite):
	...
	#The interpreter.CodeCase call is run with two sets of blank quotes
	#becuase this represents the setup and teardown, which should be run blank
	#so that those parts of the code are not run.
        for i, case in enumerate(self.cases):
            if not isinstance(case, dict):
                raise ex.SerializeException('Test cases must be dictionaries')
            self.cases[i] = interpreter.CodeCase(self.console, '',
                                                 '', **case)
    ...
    def run(self, test_name, suite_number, env=None):
        ...
        for i, case in self.enumerate_cases():
            ...
            success = self._run_case(test_name, suite_number,
                                     case, i + 1) #I need to change this to reflect my changes.
            
            if(success is None):
                log.info('Case {} has been skipped'.format(i))
                results['locked'] += 1
                continue
