...
from client.utils.doctestFlags import *
...
class CodeCase(models.Case):
	...
	setup 	  -- str; This code will print after the test has been run. There is no
		     significant implementation at this time.

        teardown  -- str; This code will print after the test has been run. There is no
		     significant implementation at this time.
	...
	This is used to remove the ">" and "." characters which are required in the .py grading
	files to show where the code to run is.


        PARAMETERS:

        code -- str; lines of interpretable code, using PS1 and PS2 prompts. This will
		     be one massive chunk of code (the input and the expected output) 
	...
	The 0th index in this list will be empty.
	The 1st index will contain the test code.
	The 2nd index will contain the expected result.
	The nth index will contain the nth line of the expected results.

	NOTE: There can be multi-line results, but only single line tests. Breaking this
	      results in unexpected behavior/failure.
...
class Console(object):
    ...
    def interpret(self):
        ...
        success &= self._interpret_lines(self._teardown) #This is fine?
    ...
    def _compare(self, expected, code):
        #print('Entered compare: {}'.format(type(self)))
        ...
        if not self.skip_locked_cases and expected.locked:
            ...
            # For the cases where testing takes place, the WithFlags class is called to decide if the code is a match, and 
            # replace the output with the output after flags have been applied (if any exist)
            # In any other case, the current code is used. 
        ...
        if(len(expected.output) > 0): #Jack
            ...
            flags = flagObj.get_flags()
            if(flags is not None and len(flags) > 0):
                correct = flagObj.decide_correct()
                if(len(expected.output) > 1):
                    actual.output = flagObj.get_output().split("\n")
                else:
                    actual.output[0] = flagObj.get_output()
                skipped = False
        if(skipped):
            correct = (expected.exception == actual.exception and expected.output_lines() == actual.output_lines())
            

        correct_legacy_exception = (actual.exception and [actual.exception_type] == expected.output_lines())
